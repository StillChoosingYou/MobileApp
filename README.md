...
`lib/core/theme/app_theme.dart` uses a deliberate placeholder: royal blue +
antique gold, generated through `ColorScheme.fromSeed`. Replace the two seed
colors in `lib/core/theme/app_theme.dart` (`AppColors.royalBlueSeed` /
`AppColors.goldSeed`) with the college's real colors and the whole app
re-themes consistently. Same for the college seal — drop it in
`assets/images/` and reference it from `RoleSelectScreen` /
`DigitalIdScreen` instead of the current icon placeholder.
